...
import { detectTradingSession, formatCurrency } from '../../lib/calculations';
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative">
          ...
          >
            ...
            <span className="text-[11px] text-muted font-bold hidden sm:inline">
              ({formatCurrency(activeAccount ? activeAccount.currentBalance : totalBalance)})
            </span>
            <ChevronDown className="w-3.5 h-3.5 text-muted" />
          </button>

          {accountDropdownOpen && (
            <div className="absolute left-0 mt-2 w-80 sm:w-96 bg-surface border border-border rounded-xl shadow-2xl py-2 z-50 animate-in fade-in zoom-in-95 duration-150">
              <div className="px-3.5 py-1 text-[10px] font-bold uppercase tracking-wider text-muted flex items-center justify-between">
                <span>Switch Trading Account</span>
                <span className="text-[10px] text-muted normal-case font-medium">Updated per P&L</span>
              </div>
              ...
              <button
                ...
                className={`w-full text-left px-3.5 py-2.5 text-xs hover:bg-black/5 dark:hover:bg-white/5 flex items-center justify-between gap-3 transition-colors ${
                  activeAccountId === 'all' ? 'text-primary font-bold bg-primary/10' : 'text-foreground'
                }`}
              >
                <div>
                  <div className="font-semibold">All Accounts Combined</div>
                  <div className="text-[10px] text-muted">Aggregated portfolio equity</div>
                </div>
              ...
              {/* Individual Accounts with Real-Time Current Balance & PnL */}
              {accounts.map(acc => {
                const pnlDiff = acc.currentBalance - (acc.initialBalance || 0);
                return (
                  <button
                    key={acc.id}
                    onClick={() => {
                  ...
                  >
                    <div className="truncate pr-1 flex-1">
                      ...
                      <div className="text-[10px] text-muted truncate">{acc.broker} ({acc.type})</div>
                    </div>
                    <div className="text-right shrink-0">
                      <div className="text-xs text-foreground font-bold whitespace-nowrap">
