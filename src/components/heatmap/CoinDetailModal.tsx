    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-in fade-in select-none">
      ...
      >
        ...
        <div className="flex items-center justify-between p-5 border-b border-border/50 bg-surface-hover/30">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs px-2 py-0.5 rounded bg-surface border border-border text-slate-600 dark:text-slate-400 font-bold">
                  #{item.rank}
                </span>
              ...
              <span className="text-xs text-slate-500 dark:text-slate-400 font-['Arial',sans-serif]">
                {item.displaySymbol}
              </span>
            </div>
          </div>
          ...
          <button
            onClick={onClose}
            className="p-1.5 hover:bg-surface-hover rounded-xl text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="p-6 space-y-6">
          {/* Main Price Card */}
          <div className="flex items-baseline justify-between p-4 rounded-xl bg-slate-50 dark:bg-white/[0.04] border border-border">
            <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="p-3 rounded-xl bg-slate-50 dark:bg-white/[0.03] border border-border/60">
              <span className="text-[11px] text-slate-500 dark:text-slate-400 font-semibold uppercase block mb-1">
                Market Cap
              </span>
          ...
          {(item.change1h !== undefined || item.change7d !== undefined) && (
            <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-white/[0.03] border border-border/60">
              <span className="text-xs text-slate-600 dark:text-slate-400 font-bold uppercase tracking-wider block mb-2">
                Multi-Timeframe Returns
              </span>
              <div className="flex items-center justify-between text-xs font-['Arial',sans-serif]">
                {item.change1h !== undefined && (
                  <div className="flex flex-col">
                    <span className="text-slate-500 dark:text-slate-400 text-[10px]">1 Hour</span>
                    <span className={`font-bold ${item.change1h >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}>
                      {item.change1h >= 0 ? `+${item.change1h.toFixed(2)}%` : `${item.change1h.toFixed(2)}%`}
                    </span>
                  </div>
                )}
                <div className="flex flex-col">
                  <span className="text-slate-500 dark:text-slate-400 text-[10px]">24 Hours</span>
                  <span className={`font-bold ${item.change24h >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}>
                    {item.change24h >= 0 ? `+${item.change24h.toFixed(2)}%` : `${item.change24h.toFixed(2)}%`}
                  </span>
                </div>
                {item.change7d !== undefined && (
                  <div className="flex flex-col">
                    <span className="text-slate-500 dark:text-slate-400 text-[10px]">7 Days</span>
                    <span className={`font-bold ${item.change7d >= 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-rose-600 dark:text-rose-400'}`}>
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-3 px-4 rounded-xl bg-primary hover:bg-primary/90 text-white font-bold text-sm shadow-lg transition-all"
          >
