...
import { EmotionalState, MistakeTag } from '../types/trade';
import {
  ArrowLeft,
  Save,
  Trash2,
  ...
  if (!trade) {
    return (
      <div className="py-20 text-center space-y-4">
        ...
        <p className="text-xs text-slate-400">The trade you requested does not exist or has been deleted.</p>
        <Link to="/trades">
    ...
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2.5">
          <Link to={`/replay?tradeId=${trade.id}`}>
            <Button size="sm" variant="secondary" icon={<PlayCircle className="w-4 h-4 text-primary-light" />}>
              Replay Trade
            </Button>
          </Link>
          <button
            ...
            className="p-2 rounded-xl text-slate-400 hover:text-rose-400 hover:bg-rose-500/10 border border-border"
          >
      ...
      <div className={`p-6 rounded-2xl border flex flex-col md:flex-row md:items-center justify-between gap-4 ${
        ...
        <div>
          ...
          <div className="flex items-center gap-4 mt-2 text-xs text-slate-300">
            <span>Lot Size: <strong className="text-white font-mono">{trade.lotSize}</strong></span>
            <span>Duration: <strong className="text-white">{trade.durationMinutes ? `${trade.durationMinutes} mins` : 'Open'}</strong></span>
            <span>Session: <strong className="text-white">{trade.session}</strong></span>
          </div>
        </div>

        <div className="text-right">
          <div className="text-xs text-slate-400 font-bold uppercase">Net Profit / Loss</div>
          <div className={`text-3xl font-black font-mono ${isWin ? 'text-emerald-400' : isLoss ? 'text-rose-400' : 'text-slate-300'}`}>
            {formatCurrency(trade.netPnl)}
          </div>
          <div className="text-xs text-slate-400 mt-0.5">
            Pips: <strong className="font-mono text-white">{trade.pips || 0}</strong>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="premium-card p-4">
          <div className="text-[10px] text-slate-400 font-bold uppercase">Entry Price</div>
          <div className="text-lg font-black text-white font-mono mt-1">{trade.entryPrice}</div>
        ...
        <div className="premium-card p-4">
          <div className="text-[10px] text-slate-400 font-bold uppercase">Exit Price</div>
          <div className="text-lg font-black text-white font-mono mt-1">{trade.exitPrice || 'Running'}</div>
          <div className="text-[10px] text-slate-500 mt-0.5">
        ...
        <div className="premium-card p-4">
          <div className="text-[10px] text-slate-400 font-bold uppercase">Stop Loss / Risk</div>
          <div className="text-lg font-black text-rose-400 font-mono mt-1">{trade.stopLoss || 'None'}</div>
          <div className="text-[10px] text-slate-400 mt-0.5">
            Planned R:R: <strong>1:{trade.plannedRR || '-'}</strong>
          </div>
        </div>

        <div className="premium-card p-4">
          <div className="text-[10px] text-slate-400 font-bold uppercase">Realized R:R</div>
          <div className="text-lg font-black text-primary-light font-mono mt-1">
      ...
      <div className="premium-card p-5 space-y-4">
        <h3 className="text-sm font-bold text-white uppercase tracking-wider">Chart Screenshot Visualizer</h3>
        
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {/* Setup / Before */}
          <div>
            <span className="text-xs font-semibold text-slate-400 mb-2 block">Before (Entry Setup)</span>
            {chartBeforeUrl ? (
              ...
              <div className="h-64 rounded-xl border border-dashed border-border flex flex-col items-center justify-center text-xs text-slate-500 bg-surface-card/30">
            ...
            <input
              ...
              className="mt-2 w-full px-3 py-1.5 rounded-lg bg-surface-card border border-border text-xs text-white focus:outline-none"
            />
          </div>

          {/* Result / After */}
          <div>
            <span className="text-xs font-semibold text-slate-400 mb-2 block">After (Outcome & Exit)</span>
            {chartAfterUrl ? (
              <div className="rounded-xl overflow-hidden border border-border bg-black/40 h-64 flex items-center justify-center">
      ...
      <form onSubmit={handleSave} className="space-y-4">
        <div className="premium-card p-5 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-semibold text-slate-400 mb-1.5">Emotional State During Trade</label>
              <select
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-400 mb-1.5">Execution Rating (1-5 Stars)</label>
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-400 mb-1.5">Entry Setup & Confluences</label>
            <textarea
              rows={3}
              value={notes}
              onChange={e => setNotes(e.target.value)}
              placeholder="Why did you take this trade? What technical confluences lined up?"
              className="w-full px-3.5 py-2.5 rounded-xl bg-surface-card border border-border text-white text-xs focus:outline-none resize-none"
            />
          </div>

          <div>
            <label className="block text-xs font-semibold text-slate-400 mb-1.5">Post-Mortem & Lessons Learned</label>
            <textarea
              rows={3}
              value={postMortem}
          ...
          <div className="flex items-center justify-between pt-2">
            {saved ? (
              <span className="text-xs font-bold text-emerald-400">Trade review updated!</span>
            ) : <div />}
