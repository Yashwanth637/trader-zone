    ...
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3">
          ...
          <div className="relative md:col-span-2">
            ...
            <input
              ...
              className="w-full pl-9 pr-3.5 py-2 rounded-xl bg-surface-card border border-border text-white text-xs focus:border-primary focus:outline-none"
            />
          </div>
          ...
          <div>
            <select
              ...
              className="w-full px-3 py-2 rounded-xl bg-surface-card border border-border text-white text-xs focus:border-primary focus:outline-none"
            >
      ...
      <div className="premium-card p-5">
        ...
        {filteredTrades.length === 0 ? (
          <div className="py-12 text-center text-xs text-slate-500 border border-dashed border-border rounded-xl">
                    ...
                    <tr key={t.id} className="hover:bg-white/5 transition-colors">
                      <td className="py-3 font-bold text-white">
                        <div className="flex items-center gap-2">
                          <Link to={`/trades/${t.id}`} className="hover:text-primary-light transition-colors">
                            {t.symbol}
                          </Link>
                          {t.strategyName && (
                            <span className="text-[10px] font-normal px-2 py-0.5 rounded-full bg-surface border border-border text-slate-400">
                              {t.strategyName}
                            </span>
                          )}
                        </div>
                        <div className="text-[10px] text-slate-500 font-mono">#{t.ticket}</div>
                      </td>
                      ...
                      <td className="py-3 font-mono text-slate-200">{t.entryPrice}</td>
                      <td className="py-3 font-mono text-slate-200">{t.exitPrice || '-'}</td>
                      ...
                      <td className="py-3">
                        {t.status === 'OPEN' ? (
                          <button
                            ...
                            className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-sky-500/20 text-sky-300 border border-sky-500/40 hover:bg-sky-500/30 transition-colors"
                          >
                            Close Trade
                          </button>
                        ) : (
