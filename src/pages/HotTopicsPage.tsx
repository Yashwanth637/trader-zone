    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-16">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2.5">
          <button
            className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold border border-orange-500/40 bg-orange-500/15 text-orange-400 shadow-sm shadow-orange-500/20"
          >
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center gap-1">
                  {isGold && (
                    <span className="w-4 h-4 rounded-full bg-amber-500/20 text-amber-400 flex items-center justify-center text-[10px] font-black mr-0.5">
                      @
                    </span>
                  )}
                  <span className={`text-xs font-bold font-mono tracking-tight ${isGold ? 'text-amber-300' : 'text-zinc-200'}`}>
      ...
      <div className="bg-[#12131a] dark:bg-[#12131a] border border-white/5 rounded-2xl px-5 py-3 flex flex-wrap items-center justify-between gap-4 shadow-sm">
            ...
            {activeSessions.map(session => (
              <div
                ...
                className={`px-2.5 py-0.5 rounded-lg text-[11px] font-bold font-mono tracking-wider transition-all ${
                  session.isOpen
                    ? 'border border-emerald-500/40 bg-emerald-500/15 text-emerald-300 shadow-[0_0_10px_rgba(16,185,129,0.15)] flex items-center gap-1'
                    : 'text-zinc-600 bg-white/[0.02]'
                }`}
                title={`${session.name} ${session.isOpen ? 'Open' : 'Closed'}`}
              >
                <span>{session.code}</span>
                {session.isOpen && <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" />}
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="flex items-center gap-5">
          <div className="flex items-center gap-2.5">
            ...
            {/* Split Bull/Bear Bar */}
            <div className="w-16 h-1.5 rounded-full bg-zinc-800 flex overflow-hidden">
          ...
          <div className="text-xs font-bold text-rose-400 font-mono tracking-wider">
            4 HIGH-IMPACT
          </div>

          <div className="flex items-center gap-1 text-xs font-bold text-amber-400 font-mono tracking-wider">
        ...
        <div className="flex items-center gap-3 text-xs font-mono">
          <span className="text-zinc-500">{lastRefreshedText}</span>
          <button
            onClick={loadLiveMarketData}
            disabled={isRefreshing}
            className="flex items-center gap-1.5 text-zinc-400 hover:text-white font-bold transition-colors disabled:opacity-50 cursor-pointer"
          >
      ...
      <div className="bg-[#12131a] dark:bg-[#12131a] border border-white/5 rounded-2xl p-6 shadow-sm">
        <div className="flex items-center justify-between pb-4 border-b border-white/5">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-bold tracking-widest text-zinc-300 uppercase font-mono">
        ...
        <div className="mt-4 divide-y divide-white/[0.03] max-h-56 overflow-y-auto pr-2 custom-scrollbar">
          {headlines.map(headline => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs md:text-sm text-zinc-300 group-hover:text-white transition-colors line-clamp-2 leading-relaxed">
                  {headline.title}
                </p>
              </div>

              <div className="text-[11px] font-mono text-zinc-500 shrink-0 mt-0.5">
                {headline.timeAgo}
              </div>
            </div>
          ))}
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          ...
          <div className="flex-1 space-y-3.5">
            ...
            <div className="flex flex-wrap items-center gap-2 text-xs font-bold font-mono">
              <span className="flex items-center gap-1.5 px-2.5 py-0.5 rounded-lg border border-amber-500/40 bg-amber-500/10 text-amber-400 tracking-wider">
            ...
            <div className="flex items-center gap-2 pt-2">
              {topStory.affectedSymbols.map(sym => (
                <span
                  key={sym}
                  className="px-2.5 py-1 rounded-lg bg-white/5 border border-white/10 text-xs font-mono font-bold text-zinc-300"
                >
                  {sym}
                </span>
              ))}
          ...
          <div className="shrink-0 flex lg:flex-col items-end justify-between gap-4 pt-4 lg:pt-0 border-t lg:border-t-0 lg:border-l border-white/5 lg:pl-8">
            <div className="text-right">
              ...
              <div className="text-4xl md:text-5xl font-black font-mono text-orange-400">
                {topStory.heat}
              </div>
              {/* Heat bar */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {gridCards.map((card) => {
          return (
            <div
              key={card.id}
              className="bg-[#12131a] dark:bg-[#12131a] border border-white/5 rounded-2xl p-6 relative overflow-hidden flex flex-col justify-between shadow-sm hover:border-white/10 transition-colors"
            >
              {/* Top Accent Line */}
              ...
              <div>
                ...
                <div className="flex flex-wrap items-center gap-2 mb-3.5 text-xs font-mono font-bold">
                  <span className="text-zinc-400 font-bold">{card.heat}°</span>

                  {card.isUserMarket && (
                    <span className="flex items-center gap-1 px-2 py-0.5 rounded-lg border border-orange-500/40 bg-orange-500/10 text-orange-400 text-[11px]">
                  ...
                  <span className="px-2 py-0.5 rounded-lg bg-white/5 text-zinc-400 text-[11px]">
                    {card.timeBadge}
                  </span>
                </div>
                ...
                <p className="text-xs md:text-sm text-zinc-300 leading-relaxed mt-2.5">
                  {card.description}
                </p>
                ...
                {card.highlightTags && card.highlightTags.length > 0 && (
                  <div className="flex flex-wrap items-center gap-2 mt-3">
                    {card.highlightTags.map(tag => (
                      <span
                        key={tag}
                        className="px-2.5 py-1 rounded-lg border border-amber-500/30 bg-amber-500/5 text-amber-300 text-xs font-mono font-semibold"
                      >
                        {tag}
                      </span>
                    ))}
                ...
                {/* Watch Advisory */}
                {card.watchCallout && (
                  <div className="mt-3.5 pt-3 border-t border-white/5 flex items-start gap-2 text-xs text-zinc-300">
              ...
              <div className="flex items-center justify-between mt-5 pt-4 border-t border-white/5">
                <div className="flex items-center gap-2">
                  {card.affectedSymbols.map(sym => (
                    <span
                      key={sym}
                      className="px-2 py-0.5 rounded-md bg-white/5 text-[11px] font-mono text-zinc-400 font-semibold"
                    >
                      {sym}
                    </span>
                  ))}
                ...
                {card.relatedAsset && (
                  <div className={`text-xs font-mono font-bold ${
                    card.relatedAsset.isPositive ? 'text-emerald-400' : 'text-rose-400'
                  }`}>
